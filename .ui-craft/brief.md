...
- ~~**2026-08-17** - Keep ASMR audio and video playing through screen lock and app backgrounding, while short and long feeds pause outside the foreground.~~ Superseded 2026-08-18 by explicit per-media background controls.
...
- ~~**2026-08-18** - Fetch ASMR authors and media one page at a time, requesting the next page only near the visible list end.~~ Superseded 2026-08-18: author metadata is a compact search index and loads once; only an author's media remains paginated.
...
- ~~**2026-08-18** - Short and long feeds always pause when their surface or the app foreground is left. ASMR audio and video each have a separate persisted background-playback toggle; only the currently playing media's enabled toggle permits playback through management views, screen lock, or app backgrounding. Switching to a feed still stops ASMR because the client owns one player.~~ Superseded 2026-08-18 by music-style audio background playback.
- **2026-08-18** - Load the complete ASMR author metadata index once per app session and search it locally; keep only each author's media list paginated. *Why:* Author names and counts are small index data, and incomplete local search makes unloaded authors impossible to find.
- **2026-08-18** - ASMR audio always continues through management, screen lock, and app backgrounding; only ASMR video keeps an explicit background-playback toggle. Leaving ASMR for a feed still stops the shared player. *Why:* Audio behaves like music, while video background playback should remain intentional.
