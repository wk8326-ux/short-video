...
## [1.6.0-beta.28] - 2026-09-23

### Changed

- 电影墙、媒体库子页面、影片详情页统一成固定的 3:2 圆角框体。上一版是「按原图比例完整显示」，
  代价是同一排里 16:9 的封面比 3:2 的矮一截、左右各留一条黑边，格子高矮不齐；现在框体尺寸固定，
  三种展示方式、详情页头图用同一个形状。
- 封面按框体裁切：3:2 的封面原样铺满；16:9 这类比 3:2 更宽的封面，居中裁掉左右两侧超出的部分，
  不改变画面比例、不动上下；竖版封面完全不裁，按原比例放进框内居中显示。
- 裁切在服务端完成，客户端拿到的就是已经裁好的 WebP，所以缩略图、详情页头图不需要各自再处理一遍，
  仍然共用同一张缓存图。

### Fixed

- 换了封面形状之后，App 会重新取一次封面而不是继续读旧形状的缓存：封面 URL 里的渲染标记跟着形状走，
  服务端的缩放缓存也按形状分键，旧的 16:9 副本不会被当成新的 3:2 副本读出来。
